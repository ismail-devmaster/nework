...
export default function PatientDetails({ patient }) {
  ...
  const togglePaymentStatus = (index) => {
    ...
    updatedPayments[index].status =
      updatedPayments[index].status === "Paid" ? "Pending" : "Paid";
        ...
        <div className="mt-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {patient.paymentHistory.map((patient, index) => (
                  <tr key={index}>
                    <td>{patient.date}</td>
                    <td>{patient.time}</td>
                    <td>{patient.doctor}</td>
                    <td>{patient.description}</td>
                    <td className="text-right">${patient.amount.toFixed(2)}</td>
                    <td className="text-right">
                      <Badge
                        variant={
                          patient.status === "Paid" ? "success" : "warning"
                        }
                      >
                        {patient.status}
                      </Badge>
